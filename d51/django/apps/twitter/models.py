...
from d51.django.apps.twitter import managers
...
class TwitterUser(models.Model):
    ...
    def update_from_twitter(self, twitter = None):
        ...
        if self.id:
            values = twitter.users.show(id=self.id)
        else:
            values = twitter.users.show(screen_name = self.screen_name)
